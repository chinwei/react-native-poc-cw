...
export default class CompaniesView extends Component {
    constructor (props) {
        ...
        var restExamples = require('../rest_examples.js');

        var handleLoginResponse = function (resp) {
            this.setState({loginStatus: resp});
        }.bind(this);

        var handleGoldenRecordResponse = function (records) {
            this.setState({records: records.hits});
        }.bind(this);

        restExamples.login()
                .then(function () {
                    handleLoginResponse('I am Logged In!');
                    restExamples.getGoldenRecords().then(handleGoldenRecordResponse);
                })
                .catch(function () {
        ...
        return (
                <View style={[styles.container]}>
                    <View style={[styles.infoBar, styles.flexCenter, styles.borderBottomLight]}>
                        <Text style={{color: '#9B9B9B', fontSize: 12}}>{this.state.loginStatus} ({this.state.records.length})</Text>
                    </View>
                    <View style={{
